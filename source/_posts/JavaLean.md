...
tags: JavaLearn
categories: Web
description: Java进阶相关资料以及简单的使用教程.
type: JavaLearn
path: /article/JavaLearn
abbrlink: 165365858
...
很多朋友想提升自己，在面试过程中无法将之前接触过的知识进行输出导致面试凉凉，也听过别人的面经
，但终究是别人的经验，只有自己总结输出才是自己的宝贵精华。笔者想通过记录自己在实际项目遇到的问题，进行总结输出文档并称成为自己的东西，也致敬给初学Javaeer的童鞋们

Gitte Start/Fork :    [疯狂点击中](https://gitee.com/fadeway32/LearnDemo)
...
## <a href='#javabase'>1.java基础</a>

## <a href='#centerTransfer'>2.中间件</a>

## <a href="#springCode">3.spring源码</a>

## <a href="#procd">4.项目实战总结</a>

## <a href="#leetCode100">5.LeetCode Top100</a>

## <a href="#network">6.网络协议</a>
...
### Synchronized的四种使用方式
synchronized(this)：当a线程执行到该语句时，锁住该语句所在对象object，其它线程无法访问object中的所有synchronized代码块。
synchronized(obj)：锁住对象obj，其它线程对obj中的所有synchronized代码块的访问被阻塞。
synchronized method()：与（1）类似，区别是（1）中线程执行到某方法中的该语句才会获得锁，而对方法加锁则是当方法调用时立刻获得锁。
synchronized static method()：当线程执行到该语句时，获得锁，所有调用该方法的其它线程阻塞，但是这个类中的其它非static声明的方法可以访问，即使这些方法是用synchronized声明的，但是static声明的方法会被阻塞；注意，这个锁与对象无关。
...
### AQS理论的数据结构
  AQS内部有3个对象，一个是state（用于计数器，类似gc的回收计数器），一个是线程标记（当前线程是谁加锁的），一个是阻塞队列。 AQS是自旋锁，在等待唤醒的时候，经常会使用自旋的方式，不停地尝试获取锁，直到被其他线程获取成功。
...
---
    public static void main(String[] args) {
        ...
        float c =0.4f;
        float d= 0.9f;
        // 这里不相等 是电脑计算是通过2进制的来的 所以是2的幂次方 才可以完全计算准确 否则采用的都是近似计算
...
## <a id ="leetCode100">LeetCode Top 100</a>
  {% post_link LeetCode01 %} LeetCode 01 two Sum
...
<h3>常见面试题</h3>
  ...
  1.为什么连接建立的时候是三次握手可以改成两次握手吗？

  三次握手改成两次握手，可能会发生死锁。如果是两次握手,服务端认为连接成功，可以发送数据帧，如果确认应答的sync+ack帧在传输中丢失，那么客户端认为没有连接成功，client会一直等待sync+Ack的应答标识，而server端在发出的数据帧 没有收到ack 重复发送同样的数据帧，从而形成死锁。 
